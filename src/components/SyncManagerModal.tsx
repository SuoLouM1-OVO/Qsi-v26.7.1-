...
export const SyncManagerModal: React.FC<SyncManagerModalProps> = ({
  ...
  const handleDataSelfCheck = async () => {
    ...
    setSyncNotice('');
    try {
      const res = await performDataSelfCheck();
      setSelfCheckResult(res);
      if (res.success && res.mergedData) {
        // Execute deep diff merge before page render, update local state immediately
        onDataReload(res.mergedData);
        setSyncNotice(
          language === 'zh'
            ? `⚡ 数据自检完成！已深度对齐 ${res.report.projectsCount} 个项目与 ${res.report.guestbookCount} 条留言，硬校验合并成功！`
            : `Data self-check complete! Deep merged ${res.report.projectsCount} projects and ${res.report.guestbookCount} messages.`
        );
      }
    } catch (err) {
      console.error('Data self-check failed:', err);
      setSyncNotice(language === 'zh' ? '❌ 数据自检异常，请重试' : 'Data self-check failed');
    } finally {
            ...
            {activeTab === 'status' && (
              <div className="space-y-4">
                ...
                <div className="pt-2 space-y-3">
                  {/* Data Self-Check & Deep Diff Audit Card */}
                  <div className="p-3.5 bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/30 dark:border-amber-500/20 rounded-xl space-y-2.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-xs font-bold text-amber-800 dark:text-amber-300 font-sans">
                        <ShieldCheck className="w-4 h-4 text-amber-600 dark:text-amber-400 stroke-[2]" />
                        <span>{language === 'zh' ? '数据自检与深层差异硬校验' : 'Data Audit & Deep Diff Merge'}</span>
                      </div>
                      <span className="text-[10px] font-mono px-2 py-0.5 bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 rounded-full border border-amber-300/40">
                        {language === 'zh' ? '防劫持模式' : 'Anti-Hijack'}
                      </span>
                    </div>

                    <p className="text-xs text-gray-600 dark:text-neutral-300 leading-relaxed font-sans">
                      {language === 'zh'
                        ? '通过对比 localStorage 快照与 Firebase / 云端最新数据，在页面重新渲染前执行深层差异合并（Deep Diff Merge）。即便网络出现劫持、中断或卡顿，也能强行校验并重载本地最新真实数据。'
                        : 'Compares local snapshot with cloud/Firebase data and performs a deep diff merge before render to hard-verify local state.'}
                    </p>

                    <button
                      type="button"
                      onClick={handleDataSelfCheck}
                      disabled={isSelfChecking}
                      className="w-full py-2.5 px-4 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-mono font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm active:scale-98 disabled:opacity-50"
                    >
                      <Sparkles className={`w-4 h-4 ${isSelfChecking ? 'animate-spin' : ''}`} />
                      <span>
                        {isSelfChecking
                          ? (language === 'zh' ? '正在执行深层差异合并与数据硬校验...' : 'Performing Deep Diff Check...')
                          : (language === 'zh' ? '执行数据自检 (硬校验合并并重载)' : 'Run Data Self-Check')}
                      </span>
                    </button>
                    ...
                    {selfCheckResult && (
                      <motion.div
                        ...
                        className="mt-2 p-3 bg-white dark:bg-neutral-900 border border-amber-200 dark:border-amber-800/60 rounded-lg text-xs font-mono space-y-1.5 text-gray-700 dark:text-neutral-300"
                      >
                        <div className="flex items-center gap-1.5 text-amber-700 dark:text-amber-400 font-bold">
                          <CheckCircle2 className="w-3.5 h-3.5" />
                          <span>{language === 'zh' ? '数据自检硬校验已成功完成！' : 'Audit & Deep Merge Succeeded!'}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-1.5 text-[11px] text-gray-600 dark:text-neutral-400 pt-1 border-t border-gray-100 dark:border-neutral-800">
                          <div>• {language === 'zh' ? '核对作品总数:' : 'Total Projects:'} <strong className="text-black dark:text-white">{selfCheckResult.report.projectsCount}</strong></div>
                          <div>• {language === 'zh' ? '重合/更新项目:' : 'Updated Items:'} <strong className="text-black dark:text-white">{selfCheckResult.report.updatedProjectsCount}</strong></div>
                          <div>• {language === 'zh' ? '点赞校准项目:' : 'Liked Calibrated:'} <strong className="text-black dark:text-white">{selfCheckResult.report.likesCount}</strong></div>
                          <div>• {language === 'zh' ? '留言合并总数:' : 'Merged Messages:'} <strong className="text-black dark:text-white">{selfCheckResult.report.guestbookCount}</strong></div>
                        </div>
                      </motion.div>
                    )}
                  </div>
